...
from drone import ProjectAirSimDrone, AdvancedAirSimDrone, AirSimDrone
...
def main():
    ...
    d: ProjectAirSimDrone | AdvancedAirSimDrone | AirSimDrone = AirSimDrone()
    ...
    if os.path.exists("TreeNode.json.gz"):
        ...
        tn = tn_cls().as_root([-x, -y, -z], [x, y, z], ml)
    if vis:
        vw = VisWindow3()
    target = [84, 69, -12]
    ...
    vw.update(tree_node=tn, path_graph=pg, path=path)
    current_index = 1
    try:
        ...
        while True:
            ...
            if reach:
                ...
                if current_index >= len(path):
                    ...
                    if vis:
                        vw.update(tree_node=tn, path_graph=pg, path=p)
